...
- **初期表示のスケルトンUI（ローディングプレースホルダー）**  
  初期表示時、PCとスマートフォン（SP）それぞれの画面幅に応じて最適なスケルトンUIを表示しています。画像やデータ取得前でもレイアウト崩れや空白を感じさせず、ユーザー体験を損なわない工夫です。PC/SPでスケルトンの枚数やサイズも自動調整され、どのデバイスでも自然なローディング体験を実現しています。
